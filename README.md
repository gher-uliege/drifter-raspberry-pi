...
The following show the difference between the old (red) and adapted text file (green) as generated by the command `diff old_file new_file`.
The file to change are chown in the second line.
...
Please make a copy for thed file to be edited first:
...
Then check the differences with `diff /etc/rc.local.bak /etc/rc.local`.
